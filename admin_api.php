...
try {
    switch($action) {
        ...
        case 'delete_user':
            ...
            $pdo->prepare("DELETE v FROM vaccinations v 
...
case 'update_incident_status':
    ...
    try {
        ...
        $stmt = $pdo->prepare("UPDATE incidents SET status = ?, notes = ?, resolved_date = ? WHERE id = ?");
    ...
    // Get all vaccinations with pet and owner info
    $stmt = $pdo->query("
        SELECT v.*, p.name as pet_name, u.name as owner_name, u.email as owner_email,
               CASE 
                   WHEN v.next_due_date < CURDATE() THEN 1 
                   ELSE 0 
               END as is_overdue,
               CASE 
                   WHEN v.next_due_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN 1 
                   ELSE 0 
               END as is_due_soon
        FROM vaccinations v 
        INNER JOIN pets p ON v.pet_id = p.id 
    ...
    foreach($vaccinations as $vacc) {
        if (!in_array($vacc['pet_id'], $vaccinatedPets)) {
...
case 'get_report':
    ...
    if ($type === 'registrations') {
        ...
        // Vaccinations by month (last 6 months)
        $stmt = $pdo->query("
            SELECT DATE_FORMAT(vaccination_date, '%b %Y') as month, COUNT(*) as count 
            FROM vaccinations 
            WHERE vaccination_date >= DATE_SUB(NOW(), INTERVAL 6 MONTH)
...
case 'export_data':
    ...
    fputcsv($output, ['ID', 'Pet', 'Vaccine Type', 'Date Given', 'Next Due', 'Created At']);
    
    $stmt = $pdo->query("
        SELECT v.id, p.name as pet, v.vaccine_type, v.vaccination_date, v.next_due_date, v.created_at 
        FROM vaccinations v 
        INNER JOIN pets p ON v.pet_id = p.id
    ");
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
